...
def train(args, hyper_net, hyper_net_ema, layer_shapes, nw, hyper_optim, g_ema, d, d_optim, img_trg, imsave_path, 
    ...
    requires_grad(hyper_net_ema, False)
    requires_grad(d, True)
    accum = 0.5 ** (32 / (10 * 1000))

    for idx in pbar:
        hyper_net.train()
        requires_grad(hyper_net, True)
        i = idx + args.start_iter
        g_factor = 5 * (1 - i / args.iter)
        style_weight = 25 * (1 - i / args.iter)
        ...
        if not idx % 3 or idx == 1:
            w_trg = torch.randn([len(img_trg), 14, 512]).to(device)
            w_aux = torch.randn_like(w_trg)
        ...
        weights = hyper_net(torch.cat([w_trg[t_w_idx1], w_aux[t_w_idx2]], 2))
        params = set_weights(layer_shapes=layer_shapes, pred_weights=weights)
        w_mixed = LatentLearner(torch.cat([w_trg[t_w_idx1], w_aux[t_w_idx2]], 2), params)
        ipol_sample, _ = g_ema([w_mixed], input_is_latent=True, return_feats=False)
        
        trg_sample.requires_grad = True
        ...
        d_loss += d_r1_loss(real_pred, trg_sample)
        ...
        for t_id in t_w_idx:
            ...
            weights = hyper_net(torch.cat([w_trg[t_id].unsqueeze(0), w_aux[t_img_id].unsqueeze(0)], 2))
            params = set_weights(layer_shapes=layer_shapes, pred_weights=weights)
            w_mixed = LatentLearner(torch.cat([w_trg[t_id].unsqueeze(0), w_aux[t_img_id].unsqueeze(0)], 2), params)
        ...
        accumulate(hyper_net_ema, hyper_net, accum)
        ...
        if i % args.img_freq == 0 and i > 0:
            with torch.set_grad_enabled(False):
                for trg_id, w_target in enumerate(w_trg):
                    weights = hyper_net(torch.cat([w_target.unsqueeze(0), w_aux[trg_id].unsqueeze(0)], 2))
                    params = set_weights(layer_shapes=layer_shapes, pred_weights=weights)
                    w_mixed = LatentLearner(torch.cat([w_target.unsqueeze(0), w_aux[trg_id].unsqueeze(0)], 2), params)
                    interpolated_sample, _ = g_ema([w_mixed], input_is_latent=True, return_feats=False)
                    utils.save_image(
...
if __name__ == "__main__":
    ...
    imsave_path = os.path.join('exp/samples', args.exp)
    model_path = os.path.join('exp/checkpoints', args.exp)
    ...
    accumulate(hyper_net_ema, hyper_net, 0)

    hyper_net_ema.eval()

    # hyper_net.load_state_dict(torch.load(f'../checkpoint/hyper-200.pt'))
    ...
    for expt_number in range(args.start, args.stop):
        ...
        _ = train(args, hyper_net, hyper_net_ema, layer_shapes, nw, hyper_optim, g_ema, d, d_optim, img_target,
                  imsave_path, model_path, expt_number, device)

        del hyper_optim, g_ema, d, d_optim

        if not expt_number % 1:
            # torch.save(
            #     {'hyper_net': hyper_net.state_dict(), 'hyper_net_ema': hyper_net_ema.state_dict()}, 
            #     f'{model_path}/hyper-{expt_number}.pt'
            # )
            torch.save(
