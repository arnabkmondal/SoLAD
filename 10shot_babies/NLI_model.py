...
def LatentLearner(x, params):
    batch_w_plus = []
    w0, b0 = params[0]
    w1, b1 = params[1]
    # w2, b2 = params[2]
    # for inp, ww0, bb0, ww1, bb1, ww2, bb2 in zip(x, w0, b0, w1, b1, w2, b2):
    for inp, ww0, bb0, ww1, bb1 in zip(x, w0, b0, w1, b1):
        w_plus = []
        for i in range(14):
            w = F.relu(F.linear(inp[i], ww0[i], bb0[i]))
            w = F.leaky_relu(F.linear(w, ww1[i], bb1[i]))
            # w = F.linear(w, ww2[i], bb2[i])
            w_plus.append(w.unsqueeze(0))
        batch_w_plus.append(torch.cat(w_plus, dim=0).unsqueeze(0))
    return torch.cat(batch_w_plus, dim=0)
